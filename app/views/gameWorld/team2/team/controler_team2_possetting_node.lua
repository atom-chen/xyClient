...
local class_controler_team2_pos = import(".controler_team2_pos")
local controler_team2_possetting_node = class("controler_team2_possetting_node")


function controler_team2_possetting_node:ctor(team_possetting_node)
	self:_initModels()
...
function controler_team2_possetting_node:addEventListener( callBack )
	self._controlerEventCallBack = callBack
end

--获取当前选中位置
function controler_team2_possetting_node:getCurSelectedPos()
	return self._controlerMap.poses[self._curSelectedPos]
end

--更新视图
function controler_team2_possetting_node:setTeam(team)
	self._team = team
	self:_updateView()
end


function controler_team2_possetting_node:_initModels()
    self._controlerMap = {}
...
--创建控制器: UI
function controler_team2_possetting_node:_createControlerForUI()
	local view_posContainer = self._posContainer

    --位置
    self._controlerMap.poses = {}
    for i=1, view_posContainer:getChildrenCount() do
	    self._controlerMap.poses[i] = class_controler_team2_pos.new(
	...
	--lipeng 2015-9-2
	--暂时只开6个位置
	for i=7, view_posContainer:getChildrenCount() do
		self._controlerMap.poses[i]:setEnabled(false)
	end
...
function controler_team2_possetting_node:_registUIEvent()

end



function controler_team2_possetting_node:_posEventListener( sender, eventName )
...
function controler_team2_possetting_node:_onPosTouchEnded( sender )
	--如果位置已经被选中
	if sender:getSelected() then
		self:_doEventCallBack(self, "posClick")
	else
...
--设置当前选中位置
function controler_team2_possetting_node:_setCurSelectedPos( posIdx )
	...
	if battleHeroManager:getLeaderPos():getHeroGUID() == NULL_GUID and 
		...
		UIManager:CreatePrompt_Operate( {
			mark = "controler_team2_possetting_node",
	        title = "提示",
	        content = "请先选择队长",
		} )
		return
...
function controler_team2_possetting_node:_doEventCallBack( sender, eventName )
...
function controler_team2_possetting_node:_updateView()
	local battleHeroManager = self._team:getBattleHeroManager()
	local view_posContainer = self._posContainer
	...
	for i=1, view_posContainer:getChildrenCount() do
		if controlerPoses[i]:isEnable() then
...
return controler_team2_possetting_node
