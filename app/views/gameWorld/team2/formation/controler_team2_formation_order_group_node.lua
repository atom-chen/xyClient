...
local class_controler_team2_formation_order_node = import(".controler_team2_formation_order_node")
local controler_team2_formation_order_group_node = class("controler_team2_formation_order_group_node")


function controler_team2_formation_order_group_node:ctor(formation_type_group_node)
	self:_initModels()
...
function controler_team2_formation_order_group_node:addEventListener( callBack )
	self._controlerEventCallBack = callBack
end

--获取当前选中位置
function controler_team2_formation_order_group_node:getCurSelectedPos()
	return self._controlerMap.poses[self._curSelectedPos]
end

--设置替补列表
function controler_team2_formation_order_group_node:setTiBuOnBattlePosIdxList(list)
	self._tiBuOnBattlePosIdxList = list
	self:_updateView()
end

--通过touch位置获取替补位置
function controler_team2_formation_order_group_node:getPosWithTouchPos( touchPos )
	local poses = self._controlerMap.poses

	for i, v in ipairs(poses) do
		if v:isEnable() and v:containsPoint(touchPos) then
			return v
		end
	end
end
...
function controler_team2_formation_order_group_node:_initModels()
    self._controlerMap = {}
...
--创建控制器: UI
function controler_team2_formation_order_group_node:_createControlerForUI()
	local view_posContainer = self._baseContainer

    --位置
    self._controlerMap.poses = {}
    for i=1, view_posContainer:getChildrenCount() do
	    self._controlerMap.poses[i] = class_controler_team2_formation_order_node.new(
	    ...
	    self._controlerMap.poses[i]:setEnable(false)
	end
...
function controler_team2_formation_order_group_node:_posEventListener( sender, eventName )
...
function controler_team2_formation_order_group_node:_onPosTouchBegan( sender )
	self:setCurSelPos(sender:getIdx())
end


function controler_team2_formation_order_group_node:_doEventCallBack( sender, eventName, eventData )
...
function controler_team2_formation_order_group_node:_updateView()
	local poses = self._controlerMap.poses
	for posIdx, posV in ipairs(poses) do
		...
		for _, tiBuV in ipairs(self._tiBuOnBattlePosIdxList) do
			...
			if posIdx == tiBuV:getTiBuPos() and posV:isEnable() then
				--更新视图
...
--设置当前选中位置
function controler_team2_formation_order_group_node:setCurSelPos( posIdx )
...
return controler_team2_formation_order_group_node
