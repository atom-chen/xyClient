...
local class_market_navigation_node = import(".market_navigation_node")
local class_market_view = import("app.views.gameWorld.market.market_scrollview.lua")

UI_Market.RESOURCE_FILENAME = "ui_instance/market/market_layer.csb"
...
function UI_Market:_createControlerForUI()
	...
	-- 导航按钮
	self._controlerMap.navigation = class_market_navigation_node.new(
		self.rootNode:getChildByName("guide"):getChildByName("navigation_node")
	)

	-- -- 滚动框
	self._controlerMap.view = class_market_view.new(
...
function UI_Market:_registGlobalEventListeners()
	...
	local configs = {
		{modelName = "market_ctrl", eventName = "clickSubTitleButtom", callBack=handler(self, self.subtitleClicked)},
