...
local controler_formation_order_node = class("controler_formation_order_node")


function controler_formation_order_node:ctor(formation_order_node, posIdx)
	self:_initModels()
...
function controler_formation_order_node:getView()
	return self._formation_order_node
end

--获取英雄icon视图
function controler_formation_order_node:getHeroIconView()
	return self._controlerMap.heroIcon:getResourceNode()
end

--目标位置是否在节点范围内
function controler_formation_order_node:containsPoint(pos)
...
function controler_formation_order_node:getTouchBeginPos()
	return self._btn:getTouchBeganPosition()
end

function controler_formation_order_node:getTouchMovePos()
	return self._btn:getTouchMovePosition()
end

function controler_formation_order_node:getTouchEndPos()
	return self._btn:getTouchEndPosition()
end

function controler_formation_order_node:setSelected(enabled)
	self._isSelected = enabled
	self._checkBox:setSelected(enabled)
	--self._img_pos_selected:setVisible(enabled)
end

function controler_formation_order_node:getSelected()
	return self._isSelected
end

function controler_formation_order_node:getIdx()
	return self._posIdx
end

--设置关联的上阵位置
function controler_formation_order_node:setBattlePos(battlePos)
...
--获取关联上阵位置
function controler_formation_order_node:getBattlePos()
	return self._battlePos
end


function controler_formation_order_node:getHero()
	return self._hero
end

function controler_formation_order_node:getPosOnWar()
	--关联的战斗位置, 因为posIdx起始索引为1
	return TIBU_START_IDX + self._posIdx - 1
end


function controler_formation_order_node:addEventListener( callBack )
	self._controlerEventCallBack = callBack
end

function controler_formation_order_node:_initModels()
    self._controlerMap = {}
...
function controler_formation_order_node:_createControlerForUI()
...
function controler_formation_order_node:_registUIEvent()
...
function controler_formation_order_node:_updateView()
	...
	self._formation_order_node:getChildByName("name"):setString("顺序"..self:getIdx())
end



return controler_formation_order_node
