...
local controler_formation_type_node = class("controler_formation_type_node")


function controler_formation_type_node:ctor(formation_type_node, posIdx)
	self:_initModels()
...
function controler_formation_type_node:getView()
	return self._formation_type_node
end

function controler_formation_type_node:setSelected(enabled)
	self._isSelected = enabled
	self._checkBox:setSelected(enabled)
	self._img_pos_selected:setVisible(enabled)
end

function controler_formation_type_node:getSelected()
	return self._isSelected
end

function controler_formation_type_node:getIdx()
	return self._posIdx
end


function controler_formation_type_node:setFormationID(formationID)
	self._formationID = formationID
	self:_updateView()
end

function controler_formation_type_node:getFormationID()
	return self._formationID
end

function controler_formation_type_node:addEventListener( callBack )
	self._controlerEventCallBack = callBack
end

function controler_formation_type_node:_initModels()
    self._controlerMap = {}
...
function controler_formation_type_node:_createControlerForUI()
...
function controler_formation_type_node:_registUIEvent()
	local posButton = self._formation_type_node:getChildByName("btn")

	local function touchEvent(sender,eventType)
        if eventType == ccui.TouchEventType.ended then
        	local formationID = self:getFormationID()
			FormationSystemInstance:sendNetMsg_changeFormation(formationID)
...
function controler_formation_type_node:_updateView()
...
return controler_formation_type_node
