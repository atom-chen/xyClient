...
local class_controler_formation_type_node = import(".controler_formation_type_node")
local controler_formation_type_group_node = class("controler_formation_type_group_node")


function controler_formation_type_group_node:ctor(formation_type_group_node)
	self:_initModels()
...
function controler_formation_type_group_node:addEventListener( callBack )
	self._controlerEventCallBack = callBack
end

--获取当前选中位置
function controler_formation_type_group_node:getCurSelectedPos()
	return self._controlerMap.poses[self._curSelectedPos]
end

--设置当前选中阵型ID
function controler_formation_type_group_node:setCurSelectedFormationID(formationID)
...
--阵型ID转换为按钮索引
function controler_formation_type_group_node:formationIDToPosIdx(id)
...
--按钮索引转换为阵型ID
function controler_formation_type_group_node:posIdxToformationID(posIdx)
	local view_posContainer = self._scrollView
...
--设置当前选中位置
function controler_formation_type_group_node:setCurSelPos( posIdx )
...
function controler_formation_type_group_node:_initModels()
    self._controlerMap = {}
...
--创建控制器: UI
function controler_formation_type_group_node:_createControlerForUI()
	local view_posContainer = self._scrollView

    --位置
    self._controlerMap.poses = {}
    for i=1, view_posContainer:getChildrenCount() do
	    self._controlerMap.poses[i] = class_controler_formation_type_node.new(
...
function controler_formation_type_group_node:_registUIEvent()

end



function controler_formation_type_group_node:_posEventListener( sender, eventName )
...
function controler_formation_type_group_node:_doEventCallBack( sender, eventName )
...
function controler_formation_type_group_node:_updateView()
...
return controler_formation_type_group_node
