...
local UI_Prompt_Operate = class("UI_Prompt_Operate", cc.load("mvc").ViewBase)

-- UI_Prompt_Operate.RESOURCE_FILENAME = "testlayer.csb"
function UI_Prompt_Operate:onCreate()
...
--设置开场动画 和 
function UI_Prompt_Operate:openAnimationEffect(  )
    --开场动画
...
function UI_Prompt_Operate:upDateShow(  )
...
--注册事件
function UI_Prompt_Operate:registerEvent(  )
    --按键1
    self.Button_Ok:addClickEventListener(function (  )
        dispatchGlobaleEvent( "PromptOperate" ,"button_click" ,1)
    end);
    --按键2
    self.Button_Cancel:addClickEventListener(function (  )
        dispatchGlobaleEvent( "PromptOperate" ,"button_click" ,2)
    end);
...
function UI_Prompt_Operate:setData( params )
    self.ShowData = params or {};
    if not self.ShowData.eventlogic then
        self.ShowData.eventlogic = "app.views.prompt.PromptOperate_logic.lua"
    end
    self.markType = self.ShowData.mark or "promptopreate";
end


return UI_Prompt_Operate
