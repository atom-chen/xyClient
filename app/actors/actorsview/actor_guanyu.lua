...
local actor_guanyu = class("actor_guanyu" ,basetemplate)

function actor_guanyu:ctor(  )
	actor_guanyu.super.ctor(self)
end

--设置角色数据
function actor_guanyu:setActorData( target )
	self.actorModel = target;
	self.Map_x = self.actorModel.Map_x;
	self.Map_y = self.actorModel.Map_y;
	self.ResConfig =  ManagerHeroRes:getHeroArmature( 20001 ,self.actorModel.attribute_camp );--getFrameShowData( self.actorModel.attribute_templateid )
...
--播放等待动画
function actor_guanyu:playAnimation_Idle(  )
...
--播放被击动作
function actor_guanyu:playAnimation_Behit(  )
...
--播放闪避动作
function actor_guanyu:playAnimation_Shanbi(  )
	local animationInfo = getEffectConfigById( self.ResConfig.shanbi )
	self.idleview:playAnimationByID( animationInfo.armaturename ,nil ,0);
end

--播放角色死亡
function actor_guanyu:playAnimationDie()
	local animationInfo = getEffectConfigById( self.ResConfig.die )
	self.idleview:playAnimationByID( animationInfo.armaturename ,nil ,0);
end

--播放入场动画
function actor_guanyu:playAnimation_Entrance(  )
	if self.idleview then
...
--播放技能攻击动作
function actor_guanyu:playAnimation_Skill_1(  )
	if self.idleview then
		self.idleview:setVisible(false);
	end
...
--技能播放结束
function actor_guanyu:playAnimation_SkillOver_1(  )
...
function actor_guanyu:playAnimation_Skill_2(  )
	if self.idleview then
		self.idleview:setVisible(false);
	end
...
--技能播放结束
function actor_guanyu:playAnimation_SkillOver_2(  )
...
--播放移动效果
function actor_guanyu:playAnimationMoveEffect( x , y )
	--移动位置
...
--
function actor_guanyu:setListen_Idle( listenerType,armatureName,movementType ,object , movementID)
...
--入场动作监听
function actor_guanyu:setListen_Entrance( listenerType,armatureName,movementType ,object,movementID)
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
			if self.idleview then
				self.idleview:setVisible(true);
...
function actor_guanyu:setListen_Skill_1( listenerType,armatureName,movementType ,object ,movementID)
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
...
function actor_guanyu:setListen_Skill_2( listenerType,armatureName,movementType ,object ,movementID)
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
