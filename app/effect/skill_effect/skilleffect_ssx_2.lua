...
local skilleffect_ssx_2 = class("skilleffect_ssx_2")

function skilleffect_ssx_2:ctor()
	-- body
end
...
function skilleffect_ssx_2:setData( parame )
...
--解析数据
function skilleffect_ssx_2:analysisData(  )
...
--创建攻击动作
function skilleffect_ssx_2:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_ssx_2:CreateAttackBlowEffect(  )
	--解析目标数据
	--解析目标计数
...
--创建飞行效果
function skilleffect_ssx_2:CreateFlyingEffect( aim )
	--得到坐标位置
	...
	local effect = EffectManager:CreateArmature( {
		name = "herores/sunshangxiang/f_ssx_s_2_1.ExportJson",--效果名称
		x = play_x,
		y = play_y,
...
--创建伤害效果
function skilleffect_ssx_2:CreateHurtEffect(  )
	--震动
...
--创建伤害逻辑
function skilleffect_ssx_2:CreateHurtLogic( pos )
	Data_Battle_Msg:analysisSkillHurtDataExecuteLogic( self.HurtData , pos );
end

--执行完成逻辑
function skilleffect_ssx_2:executeFinishLogic(  )
...
--设置完成回调函数
function skilleffect_ssx_2:setFinishCallback( callfun )
...
--攻击效果回调监听
function skilleffect_ssx_2:setAttackEffectListen( listenerType,armatureName,movementType ,object )
...
function skilleffect_ssx_2:setFlayingListen( listenerType,armatureName,movementType ,object )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
