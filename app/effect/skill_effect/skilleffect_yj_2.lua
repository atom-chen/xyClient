...
local skilleffect_yj_2 = class("skilleffect_yj_2")

function skilleffect_yj_2:ctor()
	-- body
end
...
function skilleffect_yj_2:setData( parame )
...
--解析数据
function skilleffect_yj_2:analysisData(  )
	--攻击的目标
...
--创建移动动作
function skilleffect_yj_2:CreateMoveAcion(  )
...
function skilleffect_yj_2:GoBackAction(  )
...
--创建攻击动作
function skilleffect_yj_2:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_yj_2:CreateAttackBlowEffect(  )
	
end

--创建伤害效果
function skilleffect_yj_2:CreateHurtEffect(  )
	-- self.FirstAim:doEvent("event_behit");
	dispatchGlobaleEvent( "battlefield" ,"shake" ,{0.3});
end

--创建伤害逻辑
function skilleffect_yj_2:CreateHurtLogic( pos )
	print("伤害数值")
	Data_Battle_Msg:analysisSkillHurtDataExecuteLogic( self.HurtData , pos );
end

--执行完成逻辑
function skilleffect_yj_2:executeFinishLogic(  )
	self.Release:doEvent("event_castover_2");
	self:GoBackAction(  );
end

--设置完成回调函数
function skilleffect_yj_2:setFinishCallback( callfun )
...
--移动结束监听
function skilleffect_yj_2:MoveFinishListen(  )
	self:CreateAttackEffect(  );
end

function skilleffect_yj_2:MoveFinishListen_1(  )
...
--攻击效果回调监听
function skilleffect_yj_2:setAttackEffectListen( listenerType,armatureName,movementType ,object )
