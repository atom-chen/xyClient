...
local skilleffect_zj_1 = class("skilleffect_zj_1")

function skilleffect_zj_1:ctor()
	-- body
end
...
function skilleffect_zj_1:setData( parame )
...
--创建攻击动作
function skilleffect_zj_1:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_zj_1:CreateAttackBlowEffect(  )
	--解析目标数据
	--解析目标计数
...
--创建伤害效果
function skilleffect_zj_1:CreateHurtEffect( pos )
	--震动
...
--设置完成回调函数
function skilleffect_zj_1:setFinishCallback( callfun )
	self.FinishCallback = callfun;
end

---------------------------------------------------
--攻击效果回调监听
function skilleffect_zj_1:setAttackEffectListen( listenerType,armatureName,movementType ,object )
...
function skilleffect_zj_1:setHurtCreateListen( listenerType,armatureName,movementType ,object  )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
