...
local skilleffect_hyy_2 = class("skilleffect_hyy_2")

function skilleffect_hyy_2:ctor()
	-- body
end
...
function skilleffect_hyy_2:setData( parame )
...
--创建攻击动作
function skilleffect_hyy_2:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_hyy_2:CreateAttackBlowEffect(  )
	--解析目标数据
	--解析目标计数
	...
	for i,v in ipairs(aimlist) do
		...
		if aim then
			...
			local effect = EffectManager:CreateArmature( {
				name = "herores/huangyueying/f_hyy_s_2_1.ExportJson",--效果名称
				x = x,
				y = y + 80,
...
--创建伤害效果
function skilleffect_hyy_2:CreateHurtEffect( pos )
...
--设置完成回调函数
function skilleffect_hyy_2:setFinishCallback( callfun )
	self.FinishCallback = callfun;
end

---------------------------------------------------
--攻击效果回调监听
function skilleffect_hyy_2:setAttackEffectListen( listenerType,armatureName,movementType ,object )
...
function skilleffect_hyy_2:setHurtCreateListen( listenerType,armatureName,movementType ,object  )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
