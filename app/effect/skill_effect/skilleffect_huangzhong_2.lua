...
local skilleffect_huangzhong_2 = class("skilleffect_huangzhong_2")

function skilleffect_huangzhong_2:ctor()
	-- body
end
...
function skilleffect_huangzhong_2:setData( parame )
...
--解析数据
function skilleffect_huangzhong_2:analysisData(  )
	--攻击的目标
	...
	if self.ReleaseCamp == Data_Battle.CONST_CAMP_PLAYER then
		self.startPoint_x = self.endPoint_x - 400
	else
		self.startPoint_x = self.endPoint_x + 400
	end
...
--施法聚焦
function skilleffect_huangzhong_2:CastFocus(  )
	--目标进入攻击层
...
--创建攻击动作
function skilleffect_huangzhong_2:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_huangzhong_2:CreateAttackBlowEffect(  )
	--震动
	...
	local effect = EffectManager:CreateArmature( {
		name = "herores/huangzhong/f_huangzhong_s_2_1.ExportJson",--效果名称
		x = self.endPoint_x ,
		y = self.endPoint_y + 80 ,
		zorder = ManagerBattle.CONST_COMBAT_ORDER + 1,
...
--创建伤害效果
function skilleffect_huangzhong_2:CreateHurtEffect( pos )
	
	-- self.FirstAim:doEvent("event_behit");
end

--创建伤害逻辑
function skilleffect_huangzhong_2:CreateHurtLogic( pos )
	Data_Battle_Msg:analysisSkillHurtDataExecuteLogic( self.HurtData , pos );
end

--执行完成逻辑
function skilleffect_huangzhong_2:executeFinishLogic(  )
...
--设置完成回调函数
function skilleffect_huangzhong_2:setFinishCallback( callfun )
...
--移动结束监听
function skilleffect_huangzhong_2:MoveFinishListen(  )
	-- self:CreateAttackEffect(  );
end

function skilleffect_huangzhong_2:MoveFinishListen_1(  )
...
--攻击效果回调监听
function skilleffect_huangzhong_2:setAttackEffectListen( listenerType,armatureName,movementType ,object )
...
return skilleffect_huangzhong_2;
