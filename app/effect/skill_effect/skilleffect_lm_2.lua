...
local skilleffect_lm_2 = class("skilleffect_lm_2")

function skilleffect_lm_2:ctor()
	-- body
end
...
function skilleffect_lm_2:setData( parame )
...
--解析数据
function skilleffect_lm_2:analysisData(  )
	--攻击的目标
...
--创建攻击动作
function skilleffect_lm_2:CreateAttackEffect(  )
...
--创建飞行效果
function skilleffect_lm_2:CreateFlyingEffect(  )
	...
	local effect = EffectManager:CreateArmature( {
		name = "herores/lvmeng/f_lm_s_2_1.ExportJson",--效果名称
		x = play_x,
		y = play_y,
...
--创建打击效果
function skilleffect_lm_2:CreateAttackBlowEffect(  )
	--震动
	dispatchGlobaleEvent( "battlefield" ,"shake" ,{0.2});
	EffectManager:CreatePiece_Shake_1( 0.2 );
	local effect = EffectManager:CreateArmature( {
		name = "animation/skilleffect/bloweffect_1.ExportJson",--效果名称
		x = self.endPoint_x ,
		y = self.endPoint_y,
		zorder = self.FirstAim.Map_x + self.FirstAim.Map_y + 1,
...
--创建伤害效果
function skilleffect_lm_2:CreateHurtEffect( pos )
	
	-- self.FirstAim:doEvent("event_behit");
end

--创建伤害逻辑
function skilleffect_lm_2:CreateHurtLogic( pos )
	Data_Battle_Msg:analysisSkillHurtDataExecuteLogic( self.HurtData , pos );
end

--执行完成逻辑
function skilleffect_lm_2:executeFinishLogic(  )
...
--设置完成回调函数
function skilleffect_lm_2:setFinishCallback( callfun )
...
--攻击效果回调监听
function skilleffect_lm_2:setAttackEffectListen( listenerType,armatureName,movementType ,object )
...
function skilleffect_lm_2:setHurtCreateListen( listenerType,armatureName,movementType ,object  )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
...
function skilleffect_lm_2:setFlayingListen( listenerType,armatureName,movementType ,object )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
