...
local skilleffect_zy_1 = class("skilleffect_zy_1")

function skilleffect_zy_1:ctor()
	-- body
end
...
function skilleffect_zy_1:setData( parame )
...
--创建攻击动作
function skilleffect_zy_1:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_zy_1:CreateAttackBlowEffect(  )
	--解析目标数据
	--解析目标计数
	...
	for i,v in ipairs(aimlist) do
		...
		if aim then
			...
			local effect = EffectManager:CreateArmature( {
				name = "herores/zhouyu/f_zy_s_1_1.ExportJson",--效果名称
				x = x,
				y = y,
...
--创建伤害效果
function skilleffect_zy_1:CreateHurtEffect( pos )
	--震动
...
--设置完成回调函数
function skilleffect_zy_1:setFinishCallback( callfun )
	self.FinishCallback = callfun;
end

---------------------------------------------------
--攻击效果回调监听
function skilleffect_zy_1:setAttackEffectListen( listenerType,armatureName,movementType ,object )
	...
	elseif listenerType == "FrameEvent" then
		--帧事件监听
		if movementType == "blow_10004" then
			--创建打击效果
...
function skilleffect_zy_1:setHurtCreateListen( listenerType,armatureName,movementType ,object  )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
