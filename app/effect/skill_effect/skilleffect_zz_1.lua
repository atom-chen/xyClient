...
local skilleffect_zz_1 = class("skilleffect_zz_1")

function skilleffect_zz_1:ctor()
	-- body
end
...
function skilleffect_zz_1:setData( parame )
...
--创建攻击动作
function skilleffect_zz_1:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_zz_1:CreateAttackBlowEffect(  )
	--解析目标数据
	--解析目标计数
	...
	for i,v in ipairs(aimlist) do
		...
		if aim then
			...
			local effect = EffectManager:CreateArmature( {
				name = "herores/zhangzhao/f_zz_s_1_1.ExportJson",--效果名称
				x = x,
				y = y,
...
--创建伤害效果
function skilleffect_zz_1:CreateHurtEffect( pos )
...
--设置完成回调函数
function skilleffect_zz_1:setFinishCallback( callfun )
	self.FinishCallback = callfun;
end

---------------------------------------------------
--攻击效果回调监听
function skilleffect_zz_1:setAttackEffectListen( listenerType,armatureName,movementType ,object )
	...
	elseif listenerType == "FrameEvent" then
		--帧事件监听
		if movementType == "blow_10048" then
			self:CreateAttackBlowEffect();
		end
	end
end

function skilleffect_zz_1:setHurtCreateListen( listenerType,armatureName,movementType ,object  )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
