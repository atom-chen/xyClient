...
local skilleffect_hz2_2 = class("skilleffect_hz2_2")

function skilleffect_hz2_2:ctor()
	-- body
end
...
function skilleffect_hz2_2:setData( parame )
...
--解析数据
function skilleffect_hz2_2:analysisData(  )
	--攻击的目标
...
--施法聚焦
function skilleffect_hz2_2:CastFocus(  )
	--目标进入攻击层
...
--创建攻击动作
function skilleffect_hz2_2:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_hz2_2:CreateAttackBlowEffect(  )
	--震动
...
--创建伤害效果
function skilleffect_hz2_2:CreateHurtEffect( pos )
	
	-- self.FirstAim:doEvent("event_behit");
end

--创建伤害逻辑
function skilleffect_hz2_2:CreateHurtLogic( pos )
	Data_Battle_Msg:analysisSkillHurtDataExecuteLogic( self.HurtData , pos );
end

--执行完成逻辑
function skilleffect_hz2_2:executeFinishLogic(  )
...
--设置完成回调函数
function skilleffect_hz2_2:setFinishCallback( callfun )
...
--移动结束监听
function skilleffect_hz2_2:MoveFinishListen(  )
	-- self:CreateAttackEffect(  );
end

function skilleffect_hz2_2:MoveFinishListen_1(  )
...
--攻击效果回调监听
function skilleffect_hz2_2:setAttackEffectListen( listenerType,armatureName,movementType ,object )
