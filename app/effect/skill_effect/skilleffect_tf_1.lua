...
local skilleffect_tf_1 = class("skilleffect_tf_1")

function skilleffect_tf_1:ctor()
	-- body
end
...
function skilleffect_tf_1:setData( parame )
...
--解析数据
function skilleffect_tf_1:analysisData(  )
	--攻击的目标
...
--创建移动动作
function skilleffect_tf_1:CreateMoveAcion(  )
...
function skilleffect_tf_1:GoBackAction(  )
...
--创建攻击动作
function skilleffect_tf_1:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_tf_1:CreateAttackBlowEffect(  )
	--解析目标数据
	--解析目标计数
...
--创建伤害效果
function skilleffect_tf_1:CreateHurtEffect(  )
	self.FirstAim:doEvent("event_behit");
end

--创建伤害逻辑
function skilleffect_tf_1:CreateHurtLogic( pos )
	print("CreateHurtLogic",pos)
	Data_Battle_Msg:analysisSkillHurtDataExecuteLogic( self.HurtData , pos );
end

--执行完成逻辑
function skilleffect_tf_1:executeFinishLogic(  )
	self.Release:doEvent("event_castover_1");
	self:GoBackAction(  );
end

--设置完成回调函数
function skilleffect_tf_1:setFinishCallback( callfun )
...
--移动结束监听
function skilleffect_tf_1:MoveFinishListen(  )
	self:CreateAttackEffect(  );
end

function skilleffect_tf_1:MoveFinishListen_1(  )
...
--攻击效果回调监听
function skilleffect_tf_1:setAttackEffectListen( listenerType,armatureName,movementType ,object )
...
function skilleffect_tf_1:setHurtCreateListen( listenerType,armatureName,movementType ,object  )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
