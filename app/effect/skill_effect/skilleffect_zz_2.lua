...
local skilleffect_zz_2 = class("skilleffect_zz_2")

function skilleffect_zz_2:ctor()
	-- body
end
...
function skilleffect_zz_2:setData( parame )
...
--解析数据
function skilleffect_zz_2:analysisData(  )
	--攻击的目标
	...
	if self.ReleaseCamp == Data_Battle.CONST_CAMP_PLAYER then
		self.startPoint_x = self.endPoint_x - 300
	else
		self.startPoint_x = self.endPoint_x + 300
	end
...
--施法聚焦
function skilleffect_zz_2:CastFocus(  )
	--目标进入攻击层
...
--创建攻击动作
function skilleffect_zz_2:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_zz_2:CreateAttackBlowEffect(  )
	--震动
...
--创建伤害效果
function skilleffect_zz_2:CreateHurtEffect( pos )
	--震动
	dispatchGlobaleEvent( "battlefield" ,"shake" ,{0.2});
	EffectManager:CreatePiece_Shake_1( 0.2);
...
--创建伤害逻辑
function skilleffect_zz_2:CreateHurtLogic( pos )
	Data_Battle_Msg:analysisSkillHurtDataExecuteLogic( self.HurtData , pos );
end

--执行完成逻辑
function skilleffect_zz_2:executeFinishLogic(  )
...
--设置完成回调函数
function skilleffect_zz_2:setFinishCallback( callfun )
...
--移动结束监听
function skilleffect_zz_2:MoveFinishListen(  )
	-- self:CreateAttackEffect(  );
end

function skilleffect_zz_2:MoveFinishListen_1(  )
...
--攻击效果回调监听
function skilleffect_zz_2:setAttackEffectListen( listenerType,armatureName,movementType ,object )
