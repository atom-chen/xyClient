...
local skilleffect_tsc_1 = class("skilleffect_tsc_1")

function skilleffect_tsc_1:ctor()
	-- body
end
...
function skilleffect_tsc_1:setData( parame )
...
--解析数据
function skilleffect_tsc_1:analysisData(  )
	--攻击的目标
...
--创建攻击动作
function skilleffect_tsc_1:CreateAttackEffect(  )
...
--创建飞行效果
function skilleffect_tsc_1:CreateFlyingEffect(  )
	local moveLength = self.dist - 470;
	...
	self.flyingEffect = EffectManager:CreateArmature( {
		name = "herores/taishici/f_tsc_s_1_1.ExportJson",--效果名称
		x = play_x,
		y = play_y,
...
--创建伤害效果
function skilleffect_tsc_1:CreateHurtEffect( pos )
	
	-- self.FirstAim:doEvent("event_behit");
end

--创建伤害逻辑
function skilleffect_tsc_1:CreateHurtLogic( pos )
	Data_Battle_Msg:analysisSkillHurtDataExecuteLogic( self.HurtData , pos );
end

--执行完成逻辑
function skilleffect_tsc_1:executeFinishLogic(  )
...
--设置完成回调函数
function skilleffect_tsc_1:setFinishCallback( callfun )
...
--攻击效果回调监听
function skilleffect_tsc_1:setAttackEffectListen( listenerType,armatureName,movementType ,object )
	...
	elseif listenerType == "FrameEvent" then
		...
		elseif movementType == "blow_10010" then
			self:CreateFlyingEffect(  );
		end
	end
end

function skilleffect_tsc_1:setHurtCreateListen( listenerType,armatureName,movementType ,object  )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
			self:executeFinishLogic(  );
		end
...
function skilleffect_tsc_1:setFlayingListen( listenerType,armatureName,movementType ,object )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
