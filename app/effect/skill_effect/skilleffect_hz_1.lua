...
local skilleffect_hz_1 = class("skilleffect_hz_1")

function skilleffect_hz_1:ctor()
	-- body
end
...
function skilleffect_hz_1:setData( parame )
...
--解析数据
function skilleffect_hz_1:analysisData(  )
	--攻击的目标
...
--施法聚焦
function skilleffect_hz_1:CastFocus(  )
	--目标进入攻击层
...
--创建攻击动作
function skilleffect_hz_1:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_hz_1:CreateAttackBlowEffect(  )
	--震动
...
--创建伤害效果
function skilleffect_hz_1:CreateHurtEffect( pos )
	
	-- self.FirstAim:doEvent("event_behit");
end

--创建伤害逻辑
function skilleffect_hz_1:CreateHurtLogic( pos )
	Data_Battle_Msg:analysisSkillHurtDataExecuteLogic( self.HurtData , pos );
end

--执行完成逻辑
function skilleffect_hz_1:executeFinishLogic(  )
...
--设置完成回调函数
function skilleffect_hz_1:setFinishCallback( callfun )
...
--移动结束监听
function skilleffect_hz_1:MoveFinishListen(  )
	-- self:CreateAttackEffect(  );
end

function skilleffect_hz_1:MoveFinishListen_1(  )
...
--攻击效果回调监听
function skilleffect_hz_1:setAttackEffectListen( listenerType,armatureName,movementType ,object )
...
function skilleffect_hz_1:setHurtCreateListen( listenerType,armatureName,movementType ,object  )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
