...
-- 一个通用测试效果(张角攻击_1 次效果为一个范围攻击效果)

local skilleffect_zj_2 = class("skilleffect_zj_2")

function skilleffect_zj_2:ctor()
	-- body
end
...
function skilleffect_zj_2:setData( parame )
	...
	self:CastFocus(  );
	print(self.ReleaseCamp)
end

--施法聚焦
function skilleffect_zj_2:CastFocus(  )
	--目标进入攻击层
...
--创建攻击动作
function skilleffect_zj_2:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_zj_2:CreateAttackBlowEffect(  )
	...
	effect:playAnimationByID( "Animation1" ,nil ,0);
	effect:setCallback(handler(self, self.setHurtCreateListen));
end

--创建伤害效果
function skilleffect_zj_2:CreateHurtEffect(  )
...
--设置完成回调函数
function skilleffect_zj_2:setFinishCallback( callfun )
	self.FinishCallback = callfun;
end

---------------------------------------------------
--攻击效果回调监听
function skilleffect_zj_2:setAttackEffectListen( listenerType,armatureName,movementType ,object )
...
function skilleffect_zj_2:setHurtCreateListen( listenerType,armatureName,movementType ,object  )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
