...
local skilleffect_zhenji_2 = class("skilleffect_zhenji_2")

function skilleffect_zhenji_2:ctor()
	-- body
end
...
function skilleffect_zhenji_2:setData( parame )
...
--创建攻击动作
function skilleffect_zhenji_2:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_zhenji_2:CreateAttackBlowEffect(  )
	--解析目标数据
	--解析目标计数
...
--创建伤害效果
function skilleffect_zhenji_2:CreateHurtEffect( pos )
...
--创建伤害逻辑
function skilleffect_zhenji_2:CreateHurtLogic( pos )
	print("CreateHurtLogic",pos)
	Data_Battle_Msg:analysisSkillHurtDataExecuteLogic( self.HurtData , pos );
end

--执行完成逻辑
function skilleffect_zhenji_2:executeFinishLogic(  )
...
--设置完成回调函数
function skilleffect_zhenji_2:setFinishCallback( callfun )
	self.FinishCallback = callfun;
end

---------------------------------------------------
--攻击效果回调监听
function skilleffect_zhenji_2:setAttackEffectListen( listenerType,armatureName,movementType ,object )
...
function skilleffect_zhenji_2:setHurtCreateListen( listenerType,armatureName,movementType ,object  )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
