...
local skilleffect_fz_1 = class("skilleffect_fz_1")

function skilleffect_fz_1:ctor()
	-- body
end
...
function skilleffect_fz_1:setData( parame )
...
--解析数据
function skilleffect_fz_1:analysisData(  )
	--[[技能说明
		次技能为一个攻打横排的技能
		攻打位置为对应横排的中间
	]]
...
--施法聚焦
function skilleffect_fz_1:CastFocus(  )
	--目标进入攻击层
...
--创建攻击动作
function skilleffect_fz_1:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_fz_1:CreateAttackBlowEffect(  )
	
end

--创建伤害效果
function skilleffect_fz_1:CreateHurtEffect(  )
	--震动
...
--创建伤害逻辑
function skilleffect_fz_1:CreateHurtLogic(  )
	Data_Battle_Msg:analysisSkillHurtDataExecuteLogic( self.HurtData , nil );
end

--执行完成逻辑
function skilleffect_fz_1:executeFinishLogic(  )
	self.Release:doEvent("event_castover_1");
	self:GoBackAction(  );
end

--设置完成回调函数
function skilleffect_fz_1:setFinishCallback( callfun )
...
--移动结束监听
function skilleffect_fz_1:MoveFinishListen(  )
	self:CreateAttackEffect(  );
end

function skilleffect_fz_1:MoveFinishListen_1(  )
...
--攻击效果回调监听
function skilleffect_fz_1:setAttackEffectListen( listenerType,armatureName,movementType ,object )
...
function skilleffect_fz_1:setHurtCreateListen( listenerType,armatureName,movementType ,object  )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
