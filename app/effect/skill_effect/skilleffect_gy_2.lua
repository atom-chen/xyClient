...
local skilleffect_gy_2 = class("skilleffect_gy_2")

function skilleffect_gy_2:ctor()
	-- body
end
...
function skilleffect_gy_2:setData( parame )
...
--解析数据
function skilleffect_gy_2:analysisData(  )
	--攻击的目标
	...
	self.FirstAimCamp = aimCamp;
	
end

--创建移动动作
function skilleffect_gy_2:CreateMoveAcion(  )
	print(self.FirstAim.Map_x ,self.FirstAim.Map_y ,self.FirstAim ,self.FirstAimCamp)
	...
	self.Release:ActorMoveEffect( x ,y ,0.2 ,function (  )
		print("跳跃完成")
	end)
	-- self.Release:doEvent("event_move",{Callback = handler(self, self.MoveFinishListen),x = x, y = y});
end

function skilleffect_gy_2:GoBackAction(  )
...
--创建攻击动作
function skilleffect_gy_2:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_gy_2:CreateAttackBlowEffect(  )
	--解析目标数据
	--解析目标计数
...
--创建伤害效果
function skilleffect_gy_2:CreateHurtEffect(  )
	-- self.FirstAim:doEvent("event_behit");
end

--创建伤害逻辑
function skilleffect_gy_2:CreateHurtLogic( pos )
	print("CreateHurtLogic",pos)
	Data_Battle_Msg:analysisSkillHurtDataExecuteLogic( self.HurtData , pos );
end

--执行完成逻辑
function skilleffect_gy_2:executeFinishLogic(  )
	self.Release:doEvent("event_castover_2");
	self:GoBackAction(  );
end

--设置完成回调函数
function skilleffect_gy_2:setFinishCallback( callfun )
...
--移动结束监听
function skilleffect_gy_2:MoveFinishListen(  )
	self:CreateAttackEffect(  );
end

function skilleffect_gy_2:MoveFinishListen_1(  )
...
--攻击效果回调监听
function skilleffect_gy_2:setAttackEffectListen( listenerType,armatureName,movementType ,object )
...
function skilleffect_gy_2:setHurtCreateListen( listenerType,armatureName,movementType ,object  )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
