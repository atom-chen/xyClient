...
local skilleffect_hg2_1 = class("skilleffect_hg2_1")

function skilleffect_hg2_1:ctor()
	-- body
end
...
function skilleffect_hg2_1:setData( parame )
...
--解析数据
function skilleffect_hg2_1:analysisData(  )
...
--创建攻击动作
function skilleffect_hg2_1:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_hg2_1:CreateAttackBlowEffect(  )
	...
	for i,v in ipairs(aimlist) do
		...
		if aim then
			...
			local effect = EffectManager:CreateArmature( {
				name = "herores/huanggai/f_hg_s_1_1.ExportJson",--效果名称
				x = x,
				y = y,
...
--创建伤害效果
function skilleffect_hg2_1:CreateHurtEffect( pos )
	--震动
...
--设置完成回调函数
function skilleffect_hg2_1:setFinishCallback( callfun )
	self.FinishCallback = callfun;
end

---------------------------------------------------
--攻击效果回调监听
function skilleffect_hg2_1:setAttackEffectListen( listenerType,armatureName,movementType ,object )
...
function skilleffect_hg2_1:setHurtCreateListen( listenerType,armatureName,movementType ,object  )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
