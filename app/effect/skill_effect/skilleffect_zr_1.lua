...
local skilleffect_zr_1 = class("skilleffect_zr_1")

function skilleffect_zr_1:ctor()
	-- body
end
...
function skilleffect_zr_1:setData( parame )
...
--解析数据
function skilleffect_zr_1:analysisData(  )
...
--创建攻击动作
function skilleffect_zr_1:CreateAttackEffect(  )
...
--创建打击效果
function skilleffect_zr_1:CreateAttackBlowEffect(  )
	--解析目标数据
	--解析目标计数
...
--创建飞行效果
function skilleffect_zr_1:CreateFlyingEffect( start_x , start_y , end_x ,end_y ,zorder)
...
--创建伤害效果
function skilleffect_zr_1:CreateHurtEffect( pos )
...
--设置完成回调函数
function skilleffect_zr_1:setFinishCallback( callfun )
	self.FinishCallback = callfun;
end

---------------------------------------------------
--攻击效果回调监听
function skilleffect_zr_1:setAttackEffectListen( listenerType,armatureName,movementType ,object )
...
function skilleffect_zr_1:setFlayingListen( listenerType,armatureName,movementType ,object )
	if listenerType == "AnimationEvent" then
		if movementType == ccs.MovementEventType.complete then
